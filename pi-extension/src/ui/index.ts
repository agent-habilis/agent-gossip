...
// Every square line is prefixed with this, applied once in `send()` — call
...
export function inject(text: string): boolean {
  return send("square-inject", text, true);
}
...
export function injectSilent(text: string): boolean {
  ...
  try {
    pi.sendMessage(
      { customType: "square-context", content: text, display: false },
...
export function registerMessageRenderers(pi: ExtensionAPI): void {
  ...
  pi.registerMessageRenderer("square-info", (message, _options, theme) => {
    const content = typeof message.content === "string" ? message.content : "";
    return renderMarkdown(content, theme);
  });
  pi.registerMessageRenderer("square-inject", (message, _options, theme) => {
    const content = typeof message.content === "string" ? message.content : "";
    return renderMarkdown(content, theme);
  });
  pi.registerMessageRenderer("square-block", (message) => {
    const content = typeof message.content === "string" ? message.content : "";
    return new Text(content, 0, 0);
  });
  pi.registerMessageRenderer("square-warning", (message, _options, theme) => {
    const content = typeof message.content === "string" ? message.content : "";
    return new Text(theme.fg("warning", content), 0, 0);
  });
  pi.registerMessageRenderer("square-error", (message, _options, theme) => {
    const content = typeof message.content === "string" ? message.content : "";
