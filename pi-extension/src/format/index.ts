import type { Peer, PingResult, SquareEvent } from "../types";

export function formatPresence(event: SquareEvent): string | null {
  ...
  if (event.subtype === "joined") {
    ...
    // (`/square-status`, the handover/task pickers) is where that surfaces.
...
export function formatMessage(event: SquareEvent): string | null {
  if (!event.body) return null;

  // Ambient ping/pong is handled silently (the daemon/extension auto-pongs and
  // RTT is tracked for /square-ping) — never surfaced, matching the Claude Code
...
export function formatNotice(event: SquareEvent): string | null {
  if (!event.body) return null;
  if (event.reply) {
...
export function formatState(event: SquareEvent): string {
  return `\`<${event.author}>\` changed shared state`;
}

export function formatPeerLifecycle(event: SquareEvent): string | null {
...
// parts. Shared by the `/square` roster picker and `formatMeta` so every surface
...
// mislabeling either.)
export function formatMeta(event: SquareEvent): string | null {
  ...
  if (touched.length === 0) {
    return event.display ?? `\`<${event.author}>\` changed square metadata`;
  }
...
export function formatDisplay(event: SquareEvent): string | null {
...
// Shared RTT report for /square-ping and the square_ping tool — one source so the
...
// "broadcast" when it went to the whole square — answer only if we can help;
