...
//! ([`agent_square::run_cli`]); `main` owns only process-level
...
/// destructors — otherwise `agent-square` would leave the terminal with `^C`
...
async fn main() -> Result<()> {
    ...
    tracing_subscriber::fmt()
        .with_env_filter(agent_square::log_filter())
        .with_writer(agent_square::install_log_sink())
        .with_ansi(false)
        .init();
    suppress_ctrl_c_echo();
    let result = agent_square::run_cli().await;
    agent_square::flush_log_if_pending();
    result
}
