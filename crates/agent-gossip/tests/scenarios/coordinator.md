...
mesh `--lookup mdns,dht,relay` (optionally `--advertise` so peers find it via `agent-gossip discover`);
...
persists per gossip and accumulates stale keys across runs, so the coordinator
