...
//! advertiser → directory mesh → discoverer → `room_found`/`room_lost`
...
/// frequent re-ads, and a short expiry (quick `room_lost`).
...
fn directory_advertise_then_discover() {
    ...
    let listed_id = ready["room"].as_str().expect("mesh id").to_string();
    ...
    // The advertised mesh should surface as `room_found`.
    let found = wait_for_line(&disc_log, "\"event\":\"room_found\"", CONNECT_TIMEOUT);
    let found_ok = found
        .as_deref()
    ...
    if !found_ok {
        ...
        reap(&mut advertiser);
        reap(&mut discoverer);
        panic!("discoverer never reported room_found for {listed_id}\nadv:\n{adv}\ndisc:\n{disc}");
    }

    // Advertiser exits → ads stop → the listing ages out → `room_lost`.
    reap(&mut advertiser);
    ...
    assert!(
        lost_ok,
        "discoverer never reported room_lost for {listed_id}\ndisc:\n{disc}"
    );
}
...
fn discover_stops_on_sigterm() {
    ...
    let up = wait_for_line(&disc_log, "\"event\":\"room_found\"", CONNECT_TIMEOUT).is_some();
    reap(&mut advertiser);
    if !up {
        reap(&mut discoverer);
        panic!(
