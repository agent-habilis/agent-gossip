...
impl JsonNode {
    /// Spawn `agent-gossip create`, wait for the
    ...
    assert!(
        creator
            .wait_for(CONNECT_TIMEOUT, |events| {
                events
                    .iter()
                    .any(|event| matches!(event, agent_gossip::OutputEvent::Ready { .. }))
            })
            .await,
...
/// Poll/stream parity: a `msg` returned by `agent-gossip poll` is the
