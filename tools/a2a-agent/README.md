...
[A2A](https://a2a-protocol.org) agent used to prove `ahsw a2a` bridges two real
...
there — exactly the field `ahsw a2a connect` rewrites to the local bridge so the
...
Both `ahsw` sides use the hidden `--loopback` flag, so the ticket carries the
