...
[A2A](https://a2a-protocol.org) agent used to prove `agent-gossip a2a` bridges two real
...
there — exactly the field `agent-gossip a2a connect` rewrites to the local bridge so the
...
Both `agent-gossip` sides use the hidden `--loopback` flag, so the ticket carries the
