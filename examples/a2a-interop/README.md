...
agent-gossip speaks **A2A spec v1.0** — ProtoJSON encoding,
...
| worker legs | `agent-gossip a2a status\|artifact` — the daemon is the A2A server; the agent *behind* it authors status/artifact frames, so these are CLI, not client JSON-RPC methods |
...
plane (the `--output json` stream / `agent-gossip poll`), which is why
...
Needs [bun](https://bun.sh) and a built `agent-gossip` binary
(`cargo build`; `AGENT_GOSSIP_BIN` overrides the
`target/debug/agent-gossip` default).
