//! `mesh-pipe` — a Unix pipe that carries stdin bytes over the gossip mesh.
//!
//! The **second consumer** of the `agent-habilis-mesh` engine, and a
...
#[command(
    name = "mesh-pipe",
    about = "A Unix pipe over the gossip mesh (a non-a2a consumer of agent-habilis-mesh)"
)]
struct Cli {
    #[command(subcommand)]
    command: Command,
...
enum Command {
    ...
    Listen(ListenArgs),
    /// Join the mesh and write inbound `pipe_data` frames to stdout.
    Connect(ConnectArgs),
}
...
struct ListenArgs {
    ...
    topic: Option<String>,
    /// Create a public mesh (all-on discovery) and print its id.
    #[arg(long)]
    public: bool,
    ...
    #[arg(long)]
    dht: bool,
    /// Create a mesh reachable via the default relay ladder.
    #[arg(long)]
    relay: bool,
...
impl NodeApp for PipeApp {
    fn classify(&self, _message: &Message) -> AppClass {
        ...
        // (base64, unsealed) — mesh-pipe publishes no a2a card, so it can
    ...
    async fn on_app_frame(
        ...
        match message.kind.app_tag().map(AppTag::as_str) {
            Some(tag::DATA) => match BASE64.decode(message.body.as_str()) {
                ...
                Err(error) => {
                    eprintln!("mesh-pipe: dropping undecodable pipe_data: {error}");
                }
            },
            Some(tag::EOF) => {
                let _ = self.stdout.flush().await;
...
enum Select {
    ...
    Topic(String),
    /// Create a fresh mesh over these lookups (`public` ⇒ the all-on preset).
    ...
    fn resolve(self, nickname: Option<Nickname>) -> Result<(SetupKind, Nickname, bool)> {
        match self {
            ...
            Select::Create { lookups, public } => {
                ...
                let name = MeshName::new("mesh-pipe").expect("valid mesh name");
...
fn listen_select(args: &ListenArgs) -> Result<Select> {
    match (&args.mesh, &args.topic) {
        (Some(_), Some(_)) => anyhow::bail!("pass only one of --mesh / --topic"),
        (Some(id), None) => Ok(Select::Mesh(id.clone())),
