...
[`agent-habilis-gossip`](../../agent-habilis-gossip) engine, and a deliberately
...
The engine's application seam is two traits — `MeshApp` (classify + dispatch
inbound `App` frames) and `MeshDriver` (lifecycle + typed inputs). `agent-gossip`
is one implementor (its A2A data model). `gossip-pipe` is a completely
...
- **~40 lines of app logic.** `classify` marks pipe frames ephemeral
  ...
  other `MeshApp`/`MeshDriver` hook uses the engine's **default** (no-op / `None`)
...
- **The generic outbound primitive.** `listen` emits frames through
  `agent_habilis_gossip::gossip::send_app(state, ctx, tag, to, corr, body)` —
  the engine's payload-agnostic build → sign → route helper.
- **The generic embedding facade.** It runs the event loop in-process via
  `agent_habilis_gossip::embed::MeshNode<PipeApp>` (the app-agnostic analogue of
  `agent-gossip`'s `embed::SwarmSession`).
...
```sh
# terminal 1 — send
echo 'hello over the gossip mesh' | gossip-pipe listen --topic demo

# terminal 2 — receive (prints "hello over the gossip mesh")
gossip-pipe connect --topic demo
```
...
`gossip-pipe listen` mints a loopback swarm and prints its `🐝…` id on stderr;
pass that id to `gossip-pipe connect 🐝…`. Discovery flags mirror the CLI:
...
> change). gossip-pipe marks `pipe_data`/`pipe_eof` **`sealed: false`**, so the
