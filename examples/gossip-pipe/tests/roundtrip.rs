//! End-to-end smoke test: pipe a known payload through `gossip-pipe listen` and
//! read it back out of `gossip-pipe connect`, over a real loopback gossip mesh
...
async fn bytes_roundtrip_over_the_mesh() {
    ...
    // Sender: create a loopback swarm (bare `listen`), print its id, read stdin.
    ...
    let swarm_id = tokio::time::timeout(Duration::from_secs(15), read_swarm_id(&mut listen_stderr))
        .await
