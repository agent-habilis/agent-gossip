...
//! (unseal-or-drop, no security change); gossip-pipe marks its
...
//! only means "not additionally encrypted", gossip-pipe's explicit choice (it
...
/// (`gossip-pipe: swarm 🐝…`).
async fn read_swarm_id(reader: &mut BufReader<tokio::process::ChildStderr>) -> Option<String> {
    ...
    loop {
        ...
        if let Some(rest) = line.trim().strip_prefix("gossip-pipe: swarm ") {
...
fn spawn_joiner(bin: &str, swarm_id: &str, nick: &str, pipe_stdout: bool) -> Child {
...
async fn payload_relays_across_a_circuit() {
    let bin = env!("CARGO_BIN_EXE_gossip-pipe");
    let payload = "hello from the far end of a circuit — 🐝 hop by hop\n";

    // Sender: create the loopback swarm, print its id, read stdin. Directed
    ...
    let swarm_id = tokio::time::timeout(Duration::from_secs(15), read_swarm_id(&mut sender_stderr))
        .await
        .expect("timed out waiting for the swarm id")
        .expect("sender printed a swarm id");
    ...
    // Two relays and the receiver join the same swarm. Only the receiver's
    // stdout is piped (the test reads it); the relays just forward circuits.
    let relay_a = spawn_joiner(bin, &swarm_id, "relay-a", false);
    let relay_b = spawn_joiner(bin, &swarm_id, "relay-b", false);
    let mut receiver = spawn_joiner(bin, &swarm_id, "receiver", true);
