...
//!
//! A handover is a skill-land usage of the task primitive that transfers a
//! work brief from one participant to another (the intent lives in the offer
//! body, not a wire field — the primitive has no notion of it).
//! Surfacing follows the directed-`Msg` precedent: a leg is shown and logged
//! only by its addressee and the sender's own echo — a third party relays it
//! without surfacing or retaining. These tests pin that contract end-to-end
//! through the real event loop (real iroh mesh, no subprocess), so the
//! captured `OutputEvent::Task` form is byte-identical to the
//! `--output json` wire the skills consume.
//!
...
/// isolated mesh, so one literal id is fine; a single task's legs all
...
async fn task_self_echoes_to_sender() {
    ...
    creator
        .task("ho-bob", &task_id(), TaskPhase::Offer, "brief")
        .await
        .expect("leg sent");
...
async fn task_progress_is_plumbing_not_logged() {
    ...
    bob.task(&alice_nick, &task_id(), TaskPhase::Progress, "35/100")
        .await
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn task_full_task_round_trips() {
