//! End-to-end subprocess test for `ahsw pipe listen-tcp` / `connect-tcp`
//! multiplexing: two local TCP services exposed on one pipe, forwarded to two
//! distinct local ports on the same host, driven at once through the real CLI.
//!
...
//! by `two_ports_share_one_quic_connection` in `src/pipe/tcp.rs`.
...
/// A spawned `ahsw pipe` child that is killed when the test ends (or panics).
...
fn spawn_piped(mut cmd: Command) -> (ChildGuard, Receiver<String>) {
    let mut child = cmd
        .stdout(Stdio::piped())
        .spawn()
        .expect("failed to spawn ahsw pipe process");
...
/// what the producer dials as its `listen-tcp` target.
fn spawn_mock_target(
    request: &'static [u8],
    response: &'static [u8],
...
#[test]
fn two_ports_forward_over_one_loopback_pipe() {
    // A loopback swarm — its id carries loopback lookups (no mDNS/DHT/relay), so
    // the pipe stays on this host. The daemon itself isn't consulted; the pipe
    // dials the ticket's embedded address directly. `_creator` is held so its
    // Drop kills the daemon at test end.
    let (_creator, swarm) = Node::create_named("pipe-mux");

    // Two local services the producer exposes, each with a distinct reply.
    let (r1, target_a) = spawn_mock_target(b"PING-A", b"PONG-A");
    let (r2, target_b) = spawn_mock_target(b"PING-B", b"PONG-B");

    // Producer: expose r1 and r2. `--output json` prints exactly one line — the
    // ready-to-run `ahsw pipe connect-tcp <ticket> r1 r2` command.
    ...
    let ticket_line = recv_line_containing(&producer_rx, "connect-tcp")
        .expect("producer never printed a connect-tcp command");
    let ticket = ticket_line
        .split_whitespace()
        .nth(3)
        .expect("connect-tcp line missing ticket token")
        .to_string();
