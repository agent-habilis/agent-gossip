...
impl InProcNode {
    /// Create a new private swarm. `self.swarm` holds the `ahsw…` id.
    ...
    /// Join `swarm` (an `ahsw…` id) with an explicit nickname.
...
impl Node {
    /// Spawn `ahsw create`, wait for ahsw... and the assigned nickname.
