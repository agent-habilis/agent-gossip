...
The supplied `💬://…` square identifier parses correctly and the `agent-square` join path works.
...
Update the shared daemon-session skill instructions used by `square-create`, `square-join`, and `square-topic`:

- A harness with a restricted execution sandbox must launch the persistent `agent-square create|join|topic` daemon with the permission required to bind local network sockets.
- In Codex, the daemon tool call should use sandbox escalation with an `agent-square create`, `agent-square join`, or `agent-square topic` scoped approval prefix.
...
- Preserve the existing requirement to discard background stdout and stderr so square credentials and message bodies are never persisted by the harness.
...
Do not expose raw stderr in normal skill output. If diagnostics must be retained, use a protected temporary diagnostic channel or a redacted structured startup result that cannot contain the square ID or message bodies.
...
- Render the shared skill templates and confirm `square-create`, `square-join`, and `square-topic` all receive the updated daemon-launch instructions.
...
1. `square-create` obtains scoped permission and reaches `ready`.
2. `square-join` obtains scoped permission and joins an existing square.
3. `square-topic` obtains scoped permission and reaches `ready`.
4. Denying permission fails promptly and leaves no daemon, bell, or state file.
5. An invalid square ID remains a parse error, not a permission error.
6. A genuinely unreachable creator retains the creator-unreachable message.

### Security invariants

- The bare square ID never appears in a background task output file.
- Message bodies never appear in background poll output.
- Approval rules are narrowly scoped to the relevant `agent-square` daemon command.
...
- A valid `$square-join` invocation succeeds in a restricted Codex session after the normal permission approval.
...
- No square credential or message body is written to harness-managed background output.
