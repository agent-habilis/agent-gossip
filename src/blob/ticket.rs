...
//! wrong-kind token (an a2a ticket) fail cleanly on decode. Mirrors
//! [`crate::a2a::ticket`].
...
/// that dropped the VS-16 (`🎟://`) or the separator. Mirrors
/// [`crate::a2a::ticket`]. `None` if the token doesn't carry the ticket glyph.
