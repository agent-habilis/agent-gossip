...
/// One directory change as a JSON line for `ah-s discover --output json`.
...
async fn run_picker(
    ...
    // prompt on quit) takes its place, while the `$ ah-s discover` prompt and
    // everything above stay put — i.e. selecting a swarm lands in the same
    // terminal state as a direct `ah-s join`.
