...
/// A node's-eye view of the routing mesh, ready to serialize to JSON.
...
impl LinkStateStore {
    ...
    /// Drop an origin's advertised links (e.g. when the peer leaves the swarm).
    ...
    pub(crate) fn topology(&self, self_id: EndpointId) -> Topology {
        let edges = self
            .vectors
            .values()
            .flat_map(|vector| {
