...
//! the gossip log. `port listen` proxies each inbound connection to a local TCP
//! service and prints the consumer's `ahsw port connect 🐝…` command on stdout;
//! `port connect` binds a local TCP listener and forwards each accepted
//! connection over a fresh connection to the producer. Unlike the single-shot
//! stdio [`pipe`](crate::pipe), one ticket serves many connections and the byte
//! flow is bidirectional. The ticket is a bearer capability (a random secret)
