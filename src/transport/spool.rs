...
pub(crate) const LOG_TARGET: &str = "agent_gossip::spool";
...
/// collision resistance over one swarm's frames — far below any birthday risk,
...
/// Owner-only permissions. An unpassworded swarm's `.frame` bytes are plaintext
...
/// into the `SwarmSender` so every `broadcast` mirrors to disk.
#[derive(Debug)]
...
impl Spool {
    ...
    /// wraps the writer into its [`crate::transport::SwarmSender`], drains the
...
/// Create `<root>/<swarm-prefix>/` (owner-only), spawn the writer +
...
///
/// The subdir reuses [`crate::util::swarm_prefix`] (the same filesystem-safe
/// per-swarm stem the runtime dir uses) so every peer of a swarm agrees on the
/// path from the id alone.
pub(crate) fn install(root: &Path, swarm_id: &str) -> Result<Spool> {
    use std::os::unix::fs::DirBuilderExt as _;

    let dir = root.join(crate::util::swarm_prefix(swarm_id));
    ...
    loop {
        tokio::select! {
            msg = rx.recv() => match msg {
                ...
                None => break, // SwarmSender dropped → daemon shutting down
            },
