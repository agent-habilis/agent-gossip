...
//! are resolved internally from a string (`ahsw…` / domain / git URL).
...
pub struct JoinConfig {
    /// What to join: an `ahsw…` id, a domain serving
...
pub struct CreateConfig {
    ...
    /// without its `ahsw…` id. Requires `public`. Default `false`.
    pub advertise: bool,
...
/// [`CreateError`]. `Resolve` is a bad target (an `ahsw…` id, a domain, or a
...
/// swarm's `ahsw…` id (with that count) into `directory` every
