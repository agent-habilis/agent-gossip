...
use agent_habilis_gossip::daemon::state_file::StateFile;
use agent_habilis_gossip::gossip::app::{AppClass, MeshApp};
use agent_habilis_gossip::lookup::add_peer_addr;
use agent_habilis_gossip::protocol::message::MessageBody;
use agent_habilis_gossip::protocol::swarm::SwarmName;
use agent_habilis_gossip::protocol::{AppTag, Channel, Message, MessageKind, Nickname};

#[async_trait::async_trait]
impl MeshApp for A2aApp {
...
impl MeshDriver for A2aApp {
    ...
    async fn handle_stdin(&mut self, line: &str, state: &mut EventLoopState, ctx: &HandlerCtx<'_>) {
        let out = self.output.clone();
        crate::a2a::send::handle_stdin_line(line, ctx.sender, ctx.swarm, ctx.author, state, &out)
            .await;
    }
    ...
    async fn handle_session(
        ...
        crate::a2a::send::handle_session_request(
            req, ctx.swarm, ctx.author, state, self, ctx.sender, &out,
        )
        .await
    }
    ...
    async fn handle_ipc(
        ...
        crate::a2a::ipc::handle_ipc_command(
            cmd, resp, ctx.swarm, name, ctx.author, state, self, ctx.sender, &out,
        )
        .await
    }
}
...
async fn publish_own_card(state: &mut EventLoopState, ctx: &HandlerCtx<'_>) {
    ...
    if let Err(error) = agent_habilis_gossip::gossip::broadcast_state_merge(
        ctx.swarm,
        ctx.author,
        merge,
        state,
...
fn classify(message: &Message) -> AppClass {
    ...
    // A task liveness beat: a status update marked `swarm:beat`.
...
/// `a2a_msg` is swarm broadcast chat — always loggable. `surfaceable` gates
...
fn ingest_remote_message(
    ...
    // any swarm member could drive and read a task they are not part of.
...
mod classify_tests {
    ...
    use agent_habilis_gossip::protocol::{
        AppTag, Message, MessageBody, MessageId, MessageKind, Nickname, SwarmId,
    };
    ...
    // A well-formed task-status frame (its payload's contextId names the swarm).
