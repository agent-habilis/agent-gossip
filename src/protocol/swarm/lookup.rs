...
impl DirectorySelection {
    ...
    /// command that advertises (`create`, `file send`).
    #[expect(
...
/// Resolve a transfer command's (`file`/`sh`)
...
mod lookup_tests {
    ...
    fn transfer_named_flags_restrict_to_those() {
        let opts = resolve_transfer_lookups(None, lookups(true, false, RelaySelection::Unset))
            .unwrap();
        assert!(opts.mdns && !opts.dht);
    ...
    fn transfer_swarm_id_wins_and_rejects_flags() {
        ...
        let error = resolve_transfer_lookups(Some(&id), lookups(true, false, RelaySelection::Unset))
            .unwrap_err();
