...
//! [`crate::protocol::ident`]) but, unlike a nickname, a swarm name may contain
...
/// bidi formatting, and `<` `>` `#` reserved for the `<nick>`/`#swarm` display
/// conventions). Unlike a `Nickname`, a swarm name **may contain the path
/// separators `/ \`** — it is never used in a filesystem path — so a swarm name
...
impl SwarmName {
    ...
    pub fn new(value: impl Into<String>) -> Result<Self, NameError> {
        ...
        if value.chars().any(ident::is_forbidden_swarm_name) {
    ...
    /// invalid chars ([`ident::is_forbidden_swarm_name`] —
...
#[cfg(test)]
mod tests {
    use super::{SwarmName, ident};

    #[test]
    fn from_topic_string_passthrough_and_case_preserving() {
        assert_eq!(
            SwarmName::from_topic_string("agent-habilis").as_str(),
            "agent-habilis"
        );
        assert_eq!(SwarmName::from_topic_string("Repo").as_str(), "Repo");
    }
    ...
    fn url_scheme_stripped_and_slashes_kept() {
        ...
        assert_eq!(SwarmName::from_topic_string("note:x").as_str(), "note:x");
    }
    ...
    fn http_url_drops_query_and_fragment() {
        ...
        assert_eq!(SwarmName::from_topic_string("https://x#f").as_str(), "x");
    ...
    fn forbidden_runs_collapse_to_single_dash() {
        ...
        assert_eq!(
            SwarmName::from_topic_string("my secret room").as_str(),
            "my-secret-room"
        );
        assert_eq!(SwarmName::from_topic_string("a#b").as_str(), "a-b");
        assert_eq!(SwarmName::from_topic_string("a\u{0007}b").as_str(), "a-b");
        assert_eq!(SwarmName::from_topic_string("  a/b  ").as_str(), "a/b");
    }

    #[test]
    fn overlong_name_truncates_with_ellipsis() {
        let name = SwarmName::from_topic_string(&"x".repeat(100));
    ...
    fn all_url_rfc_chars_except_hash_are_valid_names() {
        ...
        let url_specials = "-._~:/?[]@!$&'()*+,;=%";
        assert!(
            SwarmName::new(url_specials).is_ok(),
            "the URL punctuation set must be a valid swarm name"
        );
        for ch in url_specials.chars() {
            assert!(
                !ident::is_forbidden_swarm_name(ch),
                "URL char {ch:?} must be allowed in a swarm name"
            );
        }
        ...
        for ch in ['#', '<', '>'] {
            assert!(
                ident::is_forbidden_swarm_name(ch),
                "{ch:?} must stay forbidden in a swarm name"
            );
        }
    }
}
