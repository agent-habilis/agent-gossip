...
/// The per-user runtime base — every per-swarm folder lives under it.
///
/// This replaces the former hardcoded, shared `/tmp/agent-gossip`: a single
/// world-traversable base let any *other* local user enumerate swarms and read
...
/// via [`ensure_swarm_runtime_dir`] / [`ensure_parent_private`]) **and honour
/// its error** first: skipping it lets an attacker who pre-creates the base as a
/// symlink redirect a `0600` state file (which carries the swarm id + A2A bearer
...
pub(crate) fn ensure_parent_private(path: &std::path::Path) -> std::io::Result<()> {
    ...
    if let Some(parent) = path.parent() {
        ...
        // above and this create, and keeps the per-swarm subdir private too.
...
/// A swarm's runtime folder — `<runtime_base>/<swarm-prefix>/`. All of one
...
/// [`ensure_swarm_runtime_dir`] when about to *create* files under it.
#[must_use]
pub(crate) fn swarm_runtime_dir(swarm_id: &str) -> std::path::PathBuf {
    runtime_base().join(swarm_prefix(swarm_id))
}

/// Validate the private base (fail closed on a squat/symlink), then create the
/// per-swarm subdir. The choke point for writers whose target is *always* under
...
pub(crate) fn ensure_swarm_runtime_dir(swarm_id: &str) -> std::io::Result<std::path::PathBuf> {
    use std::os::unix::fs::DirBuilderExt as _;
    ensure_runtime_base()?;
    let dir = swarm_runtime_dir(swarm_id);
...
#[cfg(test)]
mod tests {
    use super::{is_under_runtime_base, runtime_base, swarm_prefix};
    use std::path::Path;
