...
/// Feed one task leg into the registry from the broadcast (`mine =
...
fn advance(rec: &mut TaskRecord, phase: TaskPhase, mine: bool) {
    ...
    match phase {
        ...
        TaskPhase::Context if matches!(rec.state, TaskState::Proposed | TaskState::Active) => {
            // A question/answer: advance into the task, flip the ball to
        ...
        TaskPhase::Confirm if rec.state == TaskState::Review => {
...
pub(crate) async fn tick_task_keepalive(
    ...
    /// `(task_id, peer, last_fraction)` for one keepalive-due task.
    ...
    for (task_id, peer, fraction) in due {
...
async fn broadcast_leg(
    ...
    let msg = Message::new_task(swarm, author, peer.clone(), task_id.clone(), phase, body)
