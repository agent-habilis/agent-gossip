...
#[derive(Default)]
pub(crate) struct ReassemblyStore {
...
impl ReassemblyStore {
    ...
    pub(crate) fn repair_tickets(&mut self, now: Instant) -> Vec<RepairTicket> {
...
/// envelope (kind/author/pubkey/swarm/ts), the concatenated body, the group as
...
impl ShardCache {
    ...
    pub(crate) fn insert(&mut self, group: ShardGroup, frames: Vec<bytes::Bytes>) {
    ...
    pub(crate) fn frames(&self, group: &ShardGroup, idxs: &[u32]) -> Vec<bytes::Bytes> {
...
mod tests {
    ...
    use crate::protocol::{Message, MessageBody, Nickname, Shard, ShardGroup, SwarmId};
    use crate::util::consts::{
