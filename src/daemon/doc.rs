...
//! [`SwarmDoc::ingest`] applies the change to a throwaway fork first and rejects
...
/// The outcome of ingesting one frame into a [`SwarmDoc`].
#[derive(Debug)]
...
pub(crate) struct SwarmDoc {
    // Boxed: an `Automerge` is large inline, and two `SwarmDoc`s live in the
...
impl SwarmDoc {
    pub(crate) fn new(gate_cards: bool) -> Self {
    ...
    /// before feeding the bytes back through [`SwarmDoc::ingest`] to apply them,
...
mod tests {
    ...
    fn null_deletes_key_and_preserves_siblings() {
        ...
        let mut doc = SwarmDoc::new(false);
        author(
            &mut doc,
            &alice,
    ...
    fn foreign_card_write_is_rejected_own_is_allowed() {
        ...
        let mut attacker = SwarmDoc::new(false);
        let bob_card = author(
            &mut attacker,
            &bob,
        ...
        let mut victim = SwarmDoc::new(true);
        ...
        let mut alice_doc = SwarmDoc::new(true);
