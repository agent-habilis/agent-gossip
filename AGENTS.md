...
- **roff man pages** (`man ah-s`) — generated by `cargo task man` into
  `target/man/*.1` from the clap command tree. The `man` task walks
  `agent_habilis_swarm::cli_command()` through `clap_mangen` in-process
  (the `tasks` crate owns the `clap_mangen` dep), so neither it nor any
  man-page code lands in the shipped `ah-s`. The release workflow
  regenerates these and bundles them into each archive; the Homebrew
  formula installs them.
...
- **Adversarial (`tests/adversarial.rs`, `--features adversarial`):** a real
  in-process attacker node injects **crafted** wire bytes via the
  injector (`SwarmSession::inject_raw` + `harness::adversarial::CraftedMsg`) — messages a
  ...
  *lack* (future-dated timestamps accepted, nickname impersonation accepted,
  ...
  `adversarial` feature is off by default (the target is `required-features`-gated,
