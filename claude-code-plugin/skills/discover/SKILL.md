---
name: discover
description: Browse squares advertising in a directory and join one. Runs `agent-square discover` under a Monitor and shows a refreshable picker; pick a square to hand off to `/square:join`.
---
...
Browsing is **always** allowed — discover joins no square, so there is no
"already in a square" guard here. Joining the square you pick is gated by
`/square:join` itself; if you are already in a square it will tell you to
`/square:leave` first.
...
**distinct description** (`square-discover`, not `square`) so `/square:leave`
never stops it and it never collides with a real square session.
...
joins no square and writes no session, so there is no `--state-file` /
`${PPID}`. Hold the Monitor's **task id** — you TaskStop it on every exit
path below.

**Fallback when Monitor is unavailable.** `square_found`/`square_lost` surface
**only** on `agent-square discover`'s live stdout stream — there is no public pull API
for them (discover joins no square, so there is no `poll` and no `--state-file`).
...
Ask whoever runs the square for its `💬…` id and use `/square:join <id>` directly.
```
and STOP. (`/square:create` and `/square:join` still work via their CLI fallback;
only the directory browse does not.)

## First render — only after the first square appears
...
The Monitor pushes one `square_found` / `square_lost` JSON line per
directory change as a notification. Wait for the first `square_found`. So
...
- First `square_found` notification → TaskStop the timer (so its `timeout`
  never fires as a stray notification later), then go to **Present the
  picker**.
- The timer fires first (no square yet) → print `💬️ no squares in #$DIR yet`
...
From the `square_found` / `square_lost` notifications seen so far, keep the
**latest** `square_found` per `square` id, then drop any id with a later
`square_lost`. Each entry has `square` (the `💬…` id), `name`, and `peers`.
These notifications **feed the picker** — do not echo them as `💬️` lines.

## Present the picker

Call `AskUserQuestion` (header `Square`):

- One option per square, **most peers first, up to 2**: label `💬 #<name>`,
  description = `<peers> peers` then the square's **full** `💬…` id
...
the top 2 squares show at once — `🔄 keep looking` / "Other" reach the rest.)

## Refresh loop

If the user picks **`🔄 keep looking`**: fold in any `square_found` /
`square_lost` notifications that arrived while the picker was open (the
Monitor kept pushing), recompute the live set, and reopen the picker.
Repeat until the user picks a square or stops.

## Join (hand off)

When the user picks a square (or pastes an id via "Other"), stop
discovering, then join it:

- TaskStop the `square-discover` Monitor.
- Invoke `/square:join <id>` with the chosen `💬…` id — **silently, no
  text before it**. That skill starts the square Monitor and writes the
...
— **TaskStop the `square-discover` Monitor** so `agent-square discover` never leaks.
...
- The browse is read-only; you are not in a square until `/square:join` runs.
