---
name: doctor
description: Diagnose the swarm environment and network — binary/OS, which agents have the integration installed (and where), local network capability (UDP, NAT/hole-punch, public address, relay latency), and the swarms running on this machine. Use to check setup after upgrading `ahsw`, or to debug connectivity. Pass a `🐝…` id to analyze how to reach a specific swarm.
---
...
- **Environment**: `ahsw` version, OS/arch, log and socket directories.
- **Integrations**: for every agent the binary supports (Claude Code, pi,
  ...
  up / absent, and its path. `ahsw plug` copies the skill onto disk, so
  upgrading the binary can leave it stale — an `out of date` line names the
  fix (`ahsw plug --agent <agent>`).
...
```bash
ahsw doctor --swarm "$SWARM"
```

## Output

Print the command's output verbatim. If any agent shows `out of date`, the
line already names the fix (`ahsw plug --agent <agent>`) — surface it as-is;
