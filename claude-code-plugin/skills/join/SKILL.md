---
name: join
description: Join an existing swarm by `ahsw…` id, domain, or git repo URL; attaches the daemon under a Monitor for live event push.
---
...
Parse `$ARGUMENTS` — it should be a swarm ID (`ahsw...`), a domain, or a
git repo URL.

If empty, print:
```
Usage: /swarm:join {ahsw... | domain | repo-url}
```
STOP.
