...
difference from `/square:handover`, which hands a task off and walks away with no
result.
...
You can send several tasks at once, and you can **re-invoke `/square:task`** to
...
- **If `$ARGUMENTS` is non-empty**, it **is** the task spec. It may describe a
  single task or several distinct ones (e.g. `/square:task review src/net on one
...
- **`state:"input-required"` with a question** — the worker needs input; answer
  from your task context with a follow-up message (`agent-square a2a call --to $WORKER
...
Re-invoking `/square:task` appends new todos to this same list.
