...
///
/// This replaces the former hardcoded, shared `/tmp/agent-mesh`: a single
...
pub fn runtime_base() -> std::path::PathBuf {
    std::path::PathBuf::from(format!("/tmp/agent-mesh-{}", current_uid()))
}
...
mod tests {
    ...
    fn runtime_base_is_deterministic_and_uid_scoped() {
        ...
        assert!(
            shown.starts_with("/tmp/agent-mesh-"),
    ...
    fn is_under_runtime_base_discriminates() {
        ...
        assert!(!is_under_runtime_base(Path::new(
            "/tmp/agent-mesh/sessions/x.json"
        )));
    }
