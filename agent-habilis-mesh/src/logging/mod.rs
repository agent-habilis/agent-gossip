...
//! [`messages`] logger on the `agent_mesh::messages` target.
...
pub fn log_filter() -> tracing_subscriber::EnvFilter {
    ...
    const SUBSYSTEMS: &str = "agent_mesh::gossip=info,\
        agent_mesh::lookup=info,\
        agent_mesh::beacon=info,\
        agent_mesh::lifecycle=info,\
        agent_mesh::directory=info,\
        agent_mesh::messages=info";
