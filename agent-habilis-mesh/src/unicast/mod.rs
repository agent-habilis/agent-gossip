...
//! (`gossip::ingest`), so signature-verify, mesh-gate, and cross-transport
...
pub(crate) const UNICAST_ALPN: &[u8] = b"agent-square/unicast/1";

/// `tracing` target for the unicast plane (matches the module path so
/// `EnvFilter` prefix-matching works, e.g. `RUST_LOG=agent_square::unicast=debug`).
pub(crate) const LOG_TARGET: &str = "agent_square::unicast";
