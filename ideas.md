...
A research catalog for future `ahsw` features, in the spirit of `forum`:
...
- **Self-selection beats assignment:** ahsw delegates by *naming* a worker;
...
`ahsw notice --swarm 🐝… --text "build green"` → `"type":"notice"`.
...
owns `topic`.)
`ahsw msg … --tag ci` / `ahsw poll … --tag ci,alerts`.
...
`ahsw status busy --note "running tests, ~4m"` / bare `ahsw status` prints
...
`ahsw board post "write lookup tests"` / `board claim b7Kq` / `board done b7Kq` / `board ls`.
...
to log convergence.
`ahsw vote open "merge strategy?" --option squash --option rebase --deadline 2m` / `vote cast <id> rebase`.
...
`ahsw card set --skill rust --skill review` / `ahsw peers --with-skill rust`.
...
ends.
`ahsw clip send` → hint → `ahsw clip recv 🐝…`.
...
`ahsw ignore add <pubkey>` / `ignore ls` / `ignore rm`.
...
`ahsw cfp "review src/net" --deadline 30s` → `ahsw bid <id> --estimate 5m` → `ahsw award <id> <nick>`.
...
`ahsw invite --ttl 1h --uses 1` → code; `ahsw join --invite <code>`.

### blob — content-addressed artifact sharing
Origin: iroh-blobs / sendme. ahsw is already on iroh, so BLAKE3-verified,
...
`ahsw blob add report.pdf` → hash; `ahsw blob get <hash>`; `ahsw blob pin <hash>`.
...
`ahsw on --tag alerts --exec ./notify.sh`.
