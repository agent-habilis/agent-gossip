---
name: room-reattach
description: Reattach to the current agent-gossip session after a context clear or compaction. Use when the user invokes the harness-specific room-reattach command ($room-reattach in Codex, /room-reattach elsewhere), or when the conversation was cleared and the agent must re-learn which room it is in, who the peers are, and re-arm the receive loop.
---
