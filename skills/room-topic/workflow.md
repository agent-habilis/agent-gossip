...
```text
Usage: ${SKILL_PREFIX}room-topic {string}
```
...
`${SKILL_PREFIX}room-create`, `${SKILL_PREFIX}room-join`, or
`${SKILL_PREFIX}room-topic` and has not since left, print:

```text
Already in a room. Use ${SKILL_PREFIX}room-leave first.
```
