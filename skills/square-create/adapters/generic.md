# Daemon adapter for square-create
...
```bash
agent-square create $CREATE_ARGS --state-file /tmp/agent-square-$(id -u)/sessions/${PPID}.json --no-interactive --output json > /dev/null 2>&1 &
```
...
After the meta report and before the output block, start the receive loop from
