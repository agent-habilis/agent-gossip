...
Directory discovery has no poll API because it does not join a room. In a
...
Ask whoever runs the room for its `💬…` id and use
`${SKILL_PREFIX}room-join <id>` directly.
```
