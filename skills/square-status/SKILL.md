---
name: square-status
description: Show peers and metadata for the current agent-square session. Use when the user invokes the harness-specific square-status command ($square-status in Codex, /square-status elsewhere) or asks who is in the square.
---

# square-status

This file is self-contained: every section it needs is below. Read nothing
else. The **Reattach** section applies only if `$SQUARE`, `$NAME`, or
`$NICKNAME` is missing; follow the **Receive loop** contract before replying
while in a square.

<!-- include path="workflow.md" -->

<!-- include path="../shared/invocation.md" -->

<!-- include path="../shared/receive-loop.md" -->

<!-- include path="../shared/events.md" -->

<!-- include path="../shared/reattach.md" -->
