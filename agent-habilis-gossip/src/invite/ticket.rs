...
//! tickets and told apart by the `kind` byte; the swarm id's `💬` is a different
//! glyph, so a swarm id fails invite decode on the prefix alone.
//!
//! Payload: `expiry(8, LE i64) ‖ flags(1) ‖ sig(64) ‖ hash_len(2, LE u16) ‖
//! swarm_hash(UTF-8) ‖ root_field`. Bit 0 of `flags` marks a password-protected
...
impl fmt::Debug for InviteTicket {
    // Redact the root — for a passwordless invite it is the swarm's join key in
    // the clear, so a stray `{:?}` must never leak it.
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        formatter
            .debug_struct("InviteTicket")
            .field("swarm_hash", &self.swarm_hash)
...
/// published swarm hash, the plaintext root, and the expiry.
fn signing_bytes(swarm_hash: &str, root: &[u8; 32], expiry: i64) -> Vec<u8> {
    let mut bytes = Vec::with_capacity(SIG_DOMAIN.len() + swarm_hash.len() + 32 + 8);
    bytes.extend_from_slice(SIG_DOMAIN);
    bytes.extend_from_slice(swarm_hash.as_bytes());
    bytes.extend_from_slice(root);
    bytes.extend_from_slice(&expiry.to_le_bytes());
    bytes
}

/// Mint a `🎟️` invite for `swarm`. `ttl_secs` `None`/`Some(0)` ⇒ no expiry,
/// else the invite expires `ttl_secs` from now. `password` (the swarm's, if it
/// has one) wraps the root so a scraped invite still needs it.
///
/// # Errors
/// This session is not the creator (holds no issuer key), or `swarm` is not
/// invite-only.
pub fn mint(swarm: &Swarm, ttl_secs: Option<u64>, password: Option<&Password>) -> Result<String> {
    let issuer_secret = swarm
        .issuer_secret()
        .context("only the creator can mint invites (this session holds no issuer key)")?;
    let root = *swarm.invite_key().context("not an invite-only swarm")?;
    let swarm_hash = swarm.to_string();
    let expiry = match ttl_secs {
        None | Some(0) => 0,
        Some(ttl) => clock::unix_secs().saturating_add(i64::try_from(ttl).unwrap_or(i64::MAX)),
    };
    let signature =
        SecretKey::from_bytes(issuer_secret).sign(&signing_bytes(&swarm_hash, &root, expiry));
    let root_field = match password {
        Some(password) => {
            let wrap_key = crypto::invite_wrap_key(password, swarm.wrap_salt());
...
impl InviteTicket {
    ...
    /// swarm password to unwrap the root).
    #[must_use]
    ...
    fn encode(&self) -> String {
        let hash_bytes = self.swarm_hash.as_bytes();
        let hash_len = u16::try_from(hash_bytes.len()).expect("swarm hash fits in a u16 length");
        let mut payload =
    ...
    pub(crate) fn decode(token: &str) -> Result<Self> {
        ...
        let hash_bytes = payload.get(pos..hash_end).context("truncated swarm hash")?;
        pos = hash_end;
        let swarm_hash = std::str::from_utf8(hash_bytes)
            .context("invalid swarm hash utf-8")?
            .to_owned();
    ...
    /// Redeem the invite: reconstruct the swarm, unwrap the root (needs the
    /// password iff protected), verify the creator's signature, and reject an
    /// expired invite. Returns a swarm ready to join.
    ///
    /// # Errors
    ...
    pub(crate) fn redeem(&self, password: Option<&Password>) -> Result<Swarm> {
        let mut swarm: Swarm = self
            .swarm_hash
            .parse()
            .context("invite carries an invalid swarm id")?;
        let issuer_pubkey = *swarm
            .issuer_pubkey()
            .context("invite is for a non-invite-only swarm")?;
        let root = self.unwrap_root(&swarm, password)?;
        ...
        if !identity::verify(
            &pubkey,
            &signing_bytes(&self.swarm_hash, &root, self.expiry),
            &signature,
        ) {
        ...
        swarm.apply_invite(root);
        Ok(swarm)
    }

    fn unwrap_root(&self, swarm: &Swarm, password: Option<&Password>) -> Result<[u8; 32]> {
...
mod tests {
    ...
    /// A fresh invite-only swarm on the creator (holds the issuer key + root).
    fn creator() -> Swarm {
        ...
        swarm.set_invite();
        assert!(
            swarm.issuer_secret().is_some(),
            "the creator holds the issuer key"
        );
        swarm
    }

    fn minted(swarm: &Swarm, ttl: Option<u64>, password: Option<&Password>) -> InviteTicket {
        InviteTicket::decode(&mint(swarm, ttl, password).unwrap()).expect("decode")
    }
    ...
    fn redeemed_invite_derives_the_creators_topic() {
        let swarm = creator();
        let redeemed = minted(&swarm, Some(3600), None).redeem(None).unwrap();
        assert!(redeemed.requires_invite());
        // The gate works: same root ⇒ same topic ⇒ they actually meet.
        assert_eq!(redeemed.topic_id(), swarm.topic_id());
    }
    ...
    fn no_expiry_invite_round_trips() {
        ...
        for ttl in [None, Some(0)] {
            let ticket = minted(&swarm, ttl, None);
    ...
    #[test]
    fn mint_refuses_a_non_invite_only_swarm() {
        let plain = Swarm::new(
            [2u8; 32],
            SwarmName::new("t").unwrap(),
            SwarmConfig::loopback(),
    ...
    fn rejects_a_zeroed_or_garbage_signature() {
        let swarm = creator();
        let mut ticket = minted(&swarm, Some(3600), None);
        ticket.sig = [0u8; 64];
    ...
    fn rejects_an_invite_signed_by_a_non_issuer_key() {
        // An attacker who knows the swarm hash + root (a member) signs an
    ...
    fn rejects_an_expired_invite_even_with_a_valid_signature() {
        ...
        let issuer = SecretKey::from_bytes(swarm.issuer_secret().unwrap());
    ...
    fn a_password_wrapped_invite_needs_the_right_password() {
        let swarm = creator();
        let ticket = minted(&swarm, Some(3600), Some(&pw("hunter2")));
        ...
        assert_eq!(ok.topic_id(), swarm.topic_id());
    }
    ...
    #[test]
    fn a_swarm_id_is_not_an_invite() {
        // A plain `💬…` id carries the swarm glyph, not the ticket `🎟️`.
        let plain = Swarm::new(
            [3u8; 32],
            SwarmName::new("t").unwrap(),
            SwarmConfig::loopback(),
