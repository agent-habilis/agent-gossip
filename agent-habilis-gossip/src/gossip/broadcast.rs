...
use crate::gossip::event::{MeshEvent, MeshSink};
use crate::protocol::identity::Identity;
use crate::protocol::{AppTag, Channel, CorrId, Message, MessageBody, Nickname, SwarmId};
...
///
/// Nor does it seal a **broadcast** body: on a passworded swarm (a
/// `broadcast_key` is set), receivers require every broadcast body to be
/// swarm-key-sealed and drop a plaintext one on decrypt (see
/// `gossip::recv::decrypt_broadcast`). A generic consumer that needs passworded
/// broadcast must therefore seal its body under the swarm key before calling
...
pub async fn send_app(
    ...
    let frame = Message::new_app(ctx.swarm, ctx.author, tag, to, corr, body).signed(ctx.identity);
...
pub async fn broadcast_msg(sender: &SwarmSender, msg: &Message) {
...
#[expect(
    ...
    reason = "the shared-state write threads swarm/author identity, the merge, the mesh state, the gossip sender and event sink, plus the target channel and the surface flag"
)]
pub async fn broadcast_state_merge(
    ...
    if surface {
        ...
        sink.emit(MeshEvent::StateChanged {
            channel,
