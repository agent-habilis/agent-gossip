...
use crate::gossip::event::MeshEvent;
use crate::lifecycle;
...
use super::app::{AppClass, MeshApp};
...
pub(crate) async fn handle_gossip_event(
    ...
    match event {
        ...
        Some(Ok(Event::NeighborUp(node_id))) => {
            ...
            if node_id == ctx.rendezvous_id {
                ...
                if !state.meshed {
                    ...
                    // (the app's card dial hint); see `MeshApp::on_meshed`.
        ...
        Some(Err(error)) => {
            ctx.sink
                .emit(MeshEvent::Error(format!("Gossip error: {error}")));
...
pub(crate) async fn ingest(
    ...
    let Ok(mut message) = Message::parse(&content) else {
        ctx.sink
            .emit(MeshEvent::Error("Failed to parse message".to_owned()));
...
fn handle_shard(
    ...
    if let Some(mut logical) = state.reassemble(message) {
        ...
        // A broadcast frame reassembles to a body that, on a passworded swarm, is
        // swarm-key sealed; decrypt it (the retained shards above stay ciphertext)
...
fn ingest_channel_event(
    ...
    match doc.ingest(message) {
        Ingested::Applied {
            ...
            ctx.sink.emit(MeshEvent::StateChanged {
                channel,
...
/// swarm-key body addressed to us, so anti-entropy re-serves the wire frame not
...
fn gate_app_payload(message: &Message, app: &dyn MeshApp) -> Option<AppClass> {
...
fn gate_and_decrypt(
    ...
    // A broadcast app frame on a passworded swarm must arrive sealed with the
    // swarm key: decrypt in place so validation + surfacing see plaintext,
    // rejecting an unsealed or unopenable body (cleartext can't slip past the
    // guarantee). Only broadcast MSG is ever swarm-key sealed — a structural
...
fn decrypt_broadcast(message: &mut Message, state: &EventLoopState) -> Option<MessageBody> {
    ...
    // A key exists (passworded swarm), so a body that will not open is either
    ...
    // did not swarm-key-seal its body lands here; see `send_app`'s doc.
    let Some(plain) = crate::daemon::state_doc::decrypt_body(message.body.as_str(), Some(key))
    else {
        tracing::warn!(
            author = %message.author,
            "dropping a broadcast body that failed swarm-key decryption (cleartext on a passworded swarm?)"
        );
        return None;
    };
...
/// App-frame loggability is the app's own per-tag policy ([`MeshApp::classify`]);
