...
use crate::daemon::app::MeshDriver;
...
/// application driver `A`. Drop it (or call [`MeshNode::leave`]) to wind the loop
/// down.
pub struct MeshNode<A: MeshDriver> {
    swarm_id: SwarmId,
    name: SwarmName,
    nickname: Nickname,
...
impl<A: MeshDriver> fmt::Debug for MeshNode<A> {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        formatter
            .debug_struct("MeshNode")
...
impl<A: MeshDriver + 'static> MeshNode<A> {
    ...
    /// [`handle_session`](MeshDriver::handle_session).
    ///
    /// # Errors
...
impl<A: MeshDriver> Drop for MeshNode<A> {
    fn drop(&mut self) {
