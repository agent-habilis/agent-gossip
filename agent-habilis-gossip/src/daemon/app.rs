...
/// [`MeshApp`] (inbound frame classification/dispatch) that adds the app's own
/// timers, driver-channel inputs, and lifecycle hooks. The loop owns the mesh
/// [`EventLoopState`] and the concrete `impl MeshDriver`; every callback hands
/// the app the loop state plus the per-arm [`HandlerCtx`].
#[async_trait::async_trait]
pub trait MeshDriver: MeshApp {
