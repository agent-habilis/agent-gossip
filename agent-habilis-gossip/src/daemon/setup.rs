...
pub async fn setup_swarm(kind: SetupKind, params: SetupParams<'_>) -> Result<EventLoopConfig> {
    let SetupParams {
        author,
        interactive,
        max_peers,
...
async fn setup_create(
    ...
    sink.emit(MeshEvent::Info(format!(
    ...
    if let Some(directory) = &advertise {
        sink.emit(MeshEvent::Info(format!("advertising on #{directory}")));
    }
    sink.emit(MeshEvent::SwarmId {
        id: swarm_id.clone(),
    });
    sink.emit(MeshEvent::Ready {
