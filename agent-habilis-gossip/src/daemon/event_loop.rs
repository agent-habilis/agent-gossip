...
use crate::gossip::event::{MeshEvent, MeshSink, PingRtt};
...
/// timers that ride this cadence run via [`MeshDriver::on_tick`] right after.
fn sweep_arm(anchors: &mut TickAnchors, state: &mut EventLoopState, sink: &dyn MeshSink) {
    timers::note_tick_gap(
        "sweep",
        &mut anchors.sweep,
...
struct EventLoop<A: MeshDriver> {
    ...
    name: SwarmName,
    author: Nickname,
    sink: std::sync::Arc<dyn MeshSink>,
    max_peers: usize,
    state: EventLoopState,
    /// Application state behind the [`MeshDriver`] seam, disjoint from the mesh
...
async fn event_loop<A: MeshDriver>(loop_state: EventLoop<A>) -> Result<()> {
    let EventLoop {
        mut sender,
        mut receiver,
        gossip,
    ...
    loop {
        tokio::select! {
            ...
            ipc_msg = recv_opt(&mut ipc_rx) => match ipc_msg {
                ...
                Some((cmd, resp_tx)) => {
                    let ctx = parts.ctx(&sender);
                    if app.handle_ipc(cmd, resp_tx, &swarm_name, &mut state, &ctx).await {
            ...
            _ = recv_opt(&mut external_quit_rx) => {
                ...
                announce_and_maybe_exit(&sender, &swarm_str, &swarm_name, &author, &mut state, &mut app, sink.as_ref(), false).await;
                break;
            }
            ...
            _ = quit_rx.recv() => {
                announce_and_maybe_exit(&sender, &swarm_str, &swarm_name, &author, &mut state, &mut app, sink.as_ref(), exit_on_quit).await;
                break;
            }
        }
        app.drain_surfaced();
...
async fn shutdown<A: MeshDriver>(
    sender: &SwarmSender,
    swarm: &SwarmId,
    name: &SwarmName,
    author: &Nickname,
    ...
    sink.emit(MeshEvent::Info(format!("left #{name}")));
...
#[expect(
    ...
    reason = "graceful shutdown threads the gossip sender, the daemon identity (swarm/name/author), the mesh state, the app state, the output sink, and the CLI hard-exit flag"
)]
async fn announce_and_maybe_exit<A: MeshDriver>(
    sender: &SwarmSender,
    swarm: &SwarmId,
    name: &SwarmName,
    author: &Nickname,
    ...
    shutdown(sender, swarm, name, author, state, app, sink).await;
...
fn spawn_ipc_rx<C: serde::de::DeserializeOwned + Send + 'static>(
    ...
    let listener = match crate::transport::ipc::bind(swarm, author) {
        ...
        Err(error) => {
            sink.emit(MeshEvent::Error(format!("IPC: {error}")));
...
fn finalize_ping_round(state: &mut EventLoopState, sink: &dyn MeshSink) {
    ...
    sink.emit(MeshEvent::PingReport { peers, known });
}
...
async fn resubscribe_tick(
    ...
    match try_resubscribe(gossip, params, state, attempts, parts.sink).await {
        ...
        Resubscribe::Fatal => {
            ...
            parts.sink.emit(MeshEvent::Error(
...
/// The line itself is broadcast by the app ([`MeshDriver::handle_stdin`]).
async fn handle_stdin_arm<A: MeshDriver>(
    ...
    match result {
        ...
        Ok(LineRead::TooLong) => {
            ctx.sink
                .emit(MeshEvent::Error("input line too long; ignored".to_owned()));
            true
        }
