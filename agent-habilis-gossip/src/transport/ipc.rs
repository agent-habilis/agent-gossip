...
/// A swarm-addressed IPC command. The engine derives the per-swarm socket path
/// from the swarm id in [`send`]; `None` means the command is addressed by
/// socket path directly (via [`send_to_path`]) and carries no swarm — the
...
pub(crate) async fn serve<C>(
    ...
    sink: std::sync::Arc<dyn crate::gossip::event::MeshSink>,
) where
    C: DeserializeOwned + Send + 'static,
{
    ...
    loop {
        match listener.accept().await {
            ...
            Err(error) => {
                if !failing {
                    sink.emit(crate::gossip::event::MeshEvent::Error(format!(
...
///
/// # Panics
/// If called with a command that carries no swarm id — never: `Info` uses `send_to_path`.
/// # Errors
    ...
    C: Serialize + Addressed,
{
    ...
    let stream = Stream::connect(name).await.map_err(|_| anyhow::anyhow!(
        "No active swarm server running for nickname '{nickname}'. Start one with `agent-gossip create` or `agent-gossip join {{{SWARM_GLYPH}...}} --nickname {nickname}`."
    ))?;
...
mod tests {
    ...
    use super::{
        Addressed, IpcMessage, Nickname, SwarmId, bind, json_error, json_ok, mpsc, send, serve,
        socket_path,
    };

    /// A minimal swarm-addressed command standing in for the app's real
    ...
    impl Addressed for TestCommand {
        fn swarm_id(&self) -> Option<&SwarmId> {
            match self {
                TestCommand::Msg { swarm, .. } | TestCommand::Ping { swarm } => Some(swarm),
            }
        }
    }
    ...
    mod prop {
        ...
        use super::{SwarmId, json_error, json_ok};
