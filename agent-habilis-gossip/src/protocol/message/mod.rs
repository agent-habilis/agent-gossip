...
///   `a2a_status`, `a2a_artifact`, `a2a_req`, `a2a_resp`) collapse into one
///   [`MessageKind::App`] carrying an opaque `tag` (the former `type`), the
...
///   peer (which would silently fail to decode an `enc` body) must NOT
///   interoperate.
///
...
/// ```json
/// {"v":"8.0","id":"<uuid>","type":"app","tag":"a2a_msg","swarm":"💬...","author":"word-word","ts":1234567890,"body":"{\"messageId\":\"<uuid>\",\"role\":\"ROLE_USER\",...}","ext":{}}
/// ```
///
...
impl Message {
    ...
    pub fn new_app(
        ...
        Self::new(swarm, author, MessageKind::App { tag, to, corr }, body)
    }

    #[must_use]
    pub fn new_joined(swarm: &SwarmId, author: &Nickname) -> Self {
    ...
    #[must_use]
    pub fn new_left(swarm: &SwarmId, author: &Nickname) -> Self {
        Self::new(
            swarm,
            author,
            MessageKind::Presence {
    ...
    #[must_use]
    pub fn new_ping(swarm: &SwarmId, author: &Nickname) -> Self {
    ...
    #[must_use]
    pub fn new_peer_info(swarm: &SwarmId, author: &Nickname, addr_data: MessageBody) -> Self {
    ...
    #[must_use]
    pub fn new_state(swarm: &SwarmId, author: &Nickname, body: MessageBody) -> Self {
...
pub fn build_msg_bytes(
    ...
    let msg = Message::new_app(swarm, author, AppTag::from("a2a_msg"), None, None, body)
...
mod tests {
    use super::{
        AppTag, ChainCtx, Message, MessageBody, MessageKind, Nickname, PresenceSubtype, SwarmId,
        build_msg_bytes,
    };
    ...
    fn test_unknown_ext_fields_ignored() {
        let json = format!(
            r#"{{"v":"8.0","id":"{FIXTURE_ID}","type":"app","tag":"a2a_msg","swarm":"💬test","author":"a-b","ts":0,"body":"hi","ext":{{"future_field":"value","another":42}}}}"#
        );
    ...
    fn test_missing_ext_defaults_to_empty_object() {
        let json = format!(
            r#"{{"v":"8.0","id":"{FIXTURE_ID}","type":"app","tag":"a2a_msg","swarm":"💬test","author":"a-b","ts":0,"body":"hi"}}"#
        );
        let parsed = Message::parse(json.as_bytes()).unwrap();
    ...
    fn parse_rejects_non_uuid_id() {
        let json = r#"{"v":"8.0","id":"not-a-uuid","type":"app","tag":"a2a_msg","swarm":"💬test","author":"a-b","ts":0,"body":"hi","ext":{}}"#;
        assert!(Message::parse(json.as_bytes()).is_err());
    }
    ...
    fn parse_rejects_control_char_body() {
        let json = format!(
            r#"{{"v":"8.0","id":"{FIXTURE_ID}","type":"app","tag":"a2a_msg","swarm":"💬test","author":"a-b","ts":0,"body":"evil\u0000body","ext":{{}}}}"#
        );
        assert!(Message::parse(json.as_bytes()).is_err());
    }
    ...
    fn parse_rejects_unsafe_author_nickname() {
        let json = format!(
            r#"{{"v":"8.0","id":"{FIXTURE_ID}","type":"app","tag":"a2a_msg","swarm":"💬test","author":"a#b","ts":0,"body":"hi","ext":{{}}}}"#
        );
        assert!(Message::parse(json.as_bytes()).is_err());
    }
    ...
    fn parse_rejects_malformed_integrity_fields() {
        ...
        let base = |extra: &str| {
            format!(
                r#"{{"v":"8.0","id":"{FIXTURE_ID}","type":"app","tag":"a2a_msg","swarm":"💬test","author":"a-b","ts":0,"body":"hi"{extra},"ext":{{}}}}"#
            )
        };
    ...
    mod snapshots {
        ...
        fn chat_fixture(text: &str) -> Message {
            ...
            let body = concat!(
                ...
                "\"contextId\":\"💬://test\",",
                "\"extensions\":[\"https://agent-habilis.dev/a2a/ext/swarm-broadcast/v1\"]}"
            );
            Message::fixture(MessageKind::app_broadcast("a2a_msg"), body)
        }
