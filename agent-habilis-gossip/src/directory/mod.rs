...
/// directory.
#[must_use]
pub fn directory_swarm(directory: &SwarmName, lookups: LookupOpts) -> Swarm {
    Swarm::new(
...
pub struct Listing {
    ...
    /// (preserved across re-ads). Display-only — the `agent-gossip discover` picker
...
impl Listings {
    ...
    pub fn expire(&mut self, ttl: Duration, now: Instant) -> Vec<SwarmId> {
    ...
    #[must_use]
    pub fn get(&self, id: &SwarmId) -> Option<&Listing> {
        self.entries.get(id)
    }
}
