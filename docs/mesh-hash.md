...
A mesh's identity is a single self-describing token — the **mesh hash**.
...
- **seed** — 32 bytes; all crypto identity (gossip topic, rendezvous
  ...
  string-derived (`SHA256(TOPIC_DOMAIN ‖ string)`) for a `topic` mesh. Either
  way the wire format below is unchanged. No peer address is ever stored, so the
  mesh is creator-independent and survives the creator's death.
...
- **config**:
  ...
  - **password verifier** (optional) — 16 bytes of one-way check value for a
    password-protected mesh: `derive_secret(K, "password-verify")[..16]`
...
a mesh cannot contain members running different lookup sets,
...
instead of silently deriving a different topic and sitting in an empty mesh.
One feature byte gates both; their fields follow in a fixed order (password
verifier, then issuer pubkey). For an **invite-only** mesh the `seed` slot is a
