...
## If two people create a gossip with the same name, is it the same gossip?

No. A gossip's identity is a **random 32-byte seed** minted at `create` time,
...
gossips that never discover or message each other.
...
The one deliberate exception is a **directory**, whose gossip seed is derived
...
[`discovery.md`](discovery.md). If your goal is "find a gossip by name," the
mechanism is advertise-into-a-directory + discover, not a name collision on the
hash.

## Can `agent-gossip discover` list two gossips with the same name?

Yes. A directory keys its listings by **gossip id**, not by name, so two gossips
...
identifier everywhere: `discover` carries both `name` and `gossip` (the id) on
every `gossip_found` line, and each is joined independently by id.
