...
## If two people create a square with the same name, is it the same square?

No. A square's identity is a **random 32-byte seed** minted at `create` time,
not its name. Two people running `agent-square create --name demo` get different seeds,
hence different `💬…` hashes and different gossip topics — two independent
squares that never discover or message each other.
...
The one deliberate exception is a **directory**, whose square seed is derived
...
meet only when they use the **same** lookups (`agent-square discover --mdns` finds an
`--mdns`-only advertiser; the all-on default on both sides meets too). That is
how `--advertise` / `agent-square discover` rendezvous by name — see
[`discovery.md`](discovery.md). If your goal is "find a square by name," the
mechanism is advertise-into-a-directory + discover, not a name collision on the
hash.

## Can `agent-square discover` list two squares with the same name?

Yes. A directory keys its listings by **square id**, not by name, so two squares
