...
hence different `ahsw…` hashes and different gossip topics — two independent
...
that a **forged** hash — someone takes a real `ahsw…` id and flips the name or a
...
both named `demo` (different seeds ⇒ different `ahsw…` ids) appear as two
separate entries that both display the name `demo`.

This is purely cosmetic in the name column — the `ahsw…` id is the real
