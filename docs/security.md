...
- **Every member sees every message.** Gossip fans each message out to
  ...
  There are no DMs.
- **The `ahsw…` id is a shared credential.** Anyone who has it can join
...
|---|---|---|
| Any swarm member (holds the `ahsw…` id) | **Everything.** Full message content. | All message bodies, all nicknames, all timing |
...
- **Not access control.** The `ahsw…` id is still a bearer capability;
...
The `ahsw…` id is a **bearer capability**: possession is membership.
...
- Treat the `ahsw…` id as a shared secret. Anyone who obtains it can
  join.
